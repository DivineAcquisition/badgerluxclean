...
export default function FinancialTab({ kpis, monthly }: Props) {
    ...
    (async () => {
      try {
        ...
        if (paid && paid.length > 0) {
          setChargesRevenue(paid.reduce((s, r) => s + (r.charge_amount || 0), 0));
        }
        ...
        if (tips && tips.length > 0) {
          setTotalTips(tips.reduce((s, r) => s + (r.tips || 0), 0));
        }
      } catch (e) {
        console.error("Financial charges query error:", e);
      }
    })();
  ...
  const variance =
    chargesRevenue !== null ? bookingsRev - chargesRevenue : null;
  const variancePct =
    chargesRevenue !== null && bookingsRev > 0
      ? Math.round(((variance ?? 0) / bookingsRev) * 1000) / 10
      : null;
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap gap-3 mt-4">
        ...
        <KPICard
          label="Total Tips"
          value={totalTips !== null ? totalTips : "—"}
          subtitle={totalTips === null ? "activates with charge data" : undefined}
        />
        <KPICard
          label="Avg Margin Per Job"
          value={
            kpis.total_bookings > 0
              ? Math.round((kpis.gross_margin / kpis.total_bookings) * 100) / 100
              : 0
          }
        />
      </div>
      ...
      <div className="flex flex-wrap gap-3 mt-4">
        <KPICard label="Bookings Revenue" value={bookingsRev} />
        <KPICard
          label="Charges Revenue"
          value={chargesRevenue !== null ? chargesRevenue : "—"}
        />
        <KPICard
        ...
        <KPICard
          ...
          value={declinedCount !== null ? declinedCount : "—"}
          color="#E74C3C"
        />
      </div>
      <div className="bg-brand-card rounded-lg p-4 mt-4 border border-brand-gold/10">
        <p className="text-xs text-gray-500 leading-relaxed">
          Revenue reconciliation compares what BookingKoala says was booked
          (Bookings_Raw) against what was actually charged (Charges_Raw).
          Variance highlights unbilled bookings or charge discrepancies. Charges
          data populates after Zapier go-live.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mt-4">
        <KPICard
          label="Leads in Archive"
          value={verification?.leadsTotal ?? "—"}
        />
        <KPICard
          label="Leads Matched to BK"
          value={verification?.leadsMatched ?? "—"}
          color="#2ECC71"
        />
        <KPICard
          label="Match Rate %"
          value={verification?.matchRate ?? "—"}
        />
        <KPICard
          label="Leads NOT in BK"
          value={verification?.leadsNotInBK ?? "—"}
          color="#F39C12"
        />
        <KPICard
          label="BK Not Tracked"
          value={verification?.bkNotTracked ?? "—"}
          color="#E74C3C"
        />
      </div>
      <div className="bg-brand-card rounded-lg p-4 mt-4 border border-brand-gold/10">
        <p className="text-xs text-gray-500 leading-relaxed font-mono">
          BookingKoala → Zapier → Google Sheets → Sync → Supabase → Dashboard
          <br />
          VAs enter leads in Lead_Tracker → archived to Leads_Archive
          <br />
          Cross-reference: Does every lead have a booking? Does every booking
          have a lead?
        </p>
      </div>
            ...
            <AreaChart data={revenueTypeData}>
              ...
              <Tooltip
                ...
                formatter={(v, name) => [
                  ...
                  name === "recur_revenue" ? "Recurring" : "One-Time",
                ]}
              />
              <Area
                type="monotone"
